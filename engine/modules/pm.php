...
if( isset( $_POST['send'] ) and !$stop_pm ) {
	...
	if( !$config['allow_comments_wysiwyg'] ) $comments = $db->safesql( $parse->BB_Parse( $parse->process( trim( $_POST['comments'] ) ), false ) );
	else {
		$parse->wysiwyg = true;
...
function reload () {
	...
	document.getElementById('dle-captcha').innerHTML = '<img src="engine/modules/antibot/antibot.php?rndval=' + rndval + '" border="0" width="160" height="80" alt="" />';
...
<script type="text/javascript" src="//www.google.com/recaptcha/api/challenge?k='.$config['recaptcha_public_key'].'"></script>' );
				...
				$tpl->set( '{sec_code}', "<a onclick=\"reload(); return false;\" href=\"#\" title=\"{$lang['reload_code']}\"><span id=\"dle-captcha\"><img src=\"" . $path['path'] . "engine/modules/antibot/antibot.php\" alt=\"{$lang['reload_code']}\" border=\"0\" width=\"160\" height=\"80\" /></span></a>" );
