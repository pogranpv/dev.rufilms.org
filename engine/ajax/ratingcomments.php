...
if( ! $user_group[$member_id['user_group']]['allow_comments_rating'] ) die( "Hacking attempt!" );
...
if( !$go_rate or !$c_id ) die( "Hacking attempt!" );
...
$row = $db->super_query( "SELECT c_id FROM " . PREFIX . "_comment_rating_log WHERE c_id ='{$c_id}' AND {$where}" );

if( !$row['c_id'] ) {

	$allrate = $db->super_query( "SELECT rating FROM " . PREFIX . "_comments WHERE id ='{$c_id}'" );
	...
	if ( $db->get_affected_rows() )	{
		...
		$db->query( "INSERT INTO " . PREFIX . "_comment_rating_log (c_id, ip, member) values ('$c_id', '$_IP', '$user_name')" );
...
$buffer = ShowCommentsRating( $row['id'], $row['rating'], $row['vote_num'], false );
