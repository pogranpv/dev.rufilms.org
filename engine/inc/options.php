...
HTML;
	...
	while ( $row = $db->get_array() ) {
		...
		if ($member_id['user_group'] == 1) $del_link = "&nbsp;&nbsp;<a href=\"#\" onclick=\"del_mod('{$row['id']}'); return false;\" class=\"status-error\"><i class=\"icon-trash\"></i>"; else $del_link = "";
	  ...
	  <div class="col-md-6">
		<div class="news with-icons">
		  ...
		  <div class="news-content">
			<div class="news-title"><a href="{$PHP_SELF}?mod={$row['name']}">{$row['title']}</a></div>
			<div class="news-text">
			  <a href="{$PHP_SELF}?mod={$row['name']}">{$row['descr']}</a>{$del_link}
			</div>
		  </div>
		</div>
	  </div>
...
elseif( $action == "dosavepersonal" ) {
	...
	if( $parse->not_allowed_tags ) {
			...
			msg( "error", $lang['user_err'], $lang['user_err_5'], "$PHP_SELF?mod=options&action=personal" );			
	...
	if( empty( $editmail ) OR strlen( $editmail ) > 50 OR @count(explode("@", $editmail)) != 2 ) {
		...
		msg( "error", $lang['user_err'], "E-mail not correct", "$PHP_SELF?mod=options&action=personal" );
	}
	...
	if( $editpassword != "" ) {
		...
		if( $altpass == $cmd5_password ) {
			...
			msg( "error", $lang['user_err'], $lang['opt_errpass'], "$PHP_SELF?mod=options&action=personal" );
...
HTML;
	...
	showRow( $lang['opt_sys_phpma'], $lang['opt_sys_phpmad'], "<input  type=\"text\" name='save_con[mail_additional]' value=\"{$config['mail_additional']}\" size=40>" );
	...
	showRow( $lang['opt_sys_usm'], $lang['opt_sys_usmd'], makeCheckBox( "save_con[use_admin_mail]", "{$config['use_admin_mail']}" ) );
...
elseif( $action == "dosavesyscon" ) {
	...
	clear_cache();
	msg( "info", $lang['opt_sysok'], $lang['opt_sysok_1'], "$PHP_SELF?mod=options&action=syscon" );
}
