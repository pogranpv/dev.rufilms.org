...
function popupedit( id ){
		...
		$('#dleuserpopup').dialog({
			...
			open: function(event, ui) {
				$("#dleuserpopup").html("<iframe name='edituserframe' id='edituserframe' width='100%' height='389' src='{$PHP_SELF}?mod=editusers&action=edituser&id=" + id + "&rndval=" + rndval + "' frameborder='0' marginwidth='0' marginheight='0' allowtransparency='true'></iframe>");
			},
...
HTML;
	...
	echo <<<HTML
<form name="searchform" id="searchform" method="post" action="$PHP_SELF?mod=editusers&action=list" class="form-horizontal">
...
HTML;
		...
		if ( count(explode("@", $row['foto'])) == 2 ) {
			$avatar = 'http://www.gravatar.com/avatar/' . md5(trim($row['foto'])) . '?s=' . intval($user_group[$row['user_group']]['max_foto']);
		} else {
			if( $row['foto'] and (file_exists( ROOT_DIR . "/uploads/fotos/" . $row['foto'] )) ) $avatar = $config['http_home_url'] . "uploads/fotos/" . $row['foto'];
...
function confirmDelete(url){
	...
	parent.DLEconfirm( '{$lang['user_deluser']}', '{$lang['p_confirm']}', function () {
		...
		document.location='{$PHP_SELF}?mod=editusers&action=dodeleteuser&popup=yes&id={$row['user_id']}&user_hash='+url;
...
elseif( $action == "dodeleteuser" ) {
	...
	msg( "info", $lang['user_delok'], $lang['news_alldel'], "$PHP_SELF?mod=editusers&action=list" );
}
